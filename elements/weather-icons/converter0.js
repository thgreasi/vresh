...
Object.keys(iconmap).forEach(function (k) {
    iconmap2[k] = $.parseHTML(iconmap[k])[0].textContent;
});

var iconmap2keys = Object.keys(iconmap2);
var glyphs = Array.slice(xml.children[0].children).map(function (g) {
    var unicode = g.attributes['unicode'] && g.attributes['unicode'].value;
    if (unicode) {
        // console.log(unicode);
        var id = iconmap2keys.filter(function (k) {
            return iconmap2[k] === unicode;
        })[0];
        // console.log(id);
        if (id) {
            return g.outerHTML.replace('<glyph ', '<g id="' + id + '"><path ').replace('/>', '></path></g>');
        }
    }
}).filter(function (x) {
    return x;
});
//# sourceMappingURL=converter0.js.map
